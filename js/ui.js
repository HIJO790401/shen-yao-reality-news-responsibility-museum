(function () {
  ...
  header.innerHTML = `
    <div class="container nav-wrap">
      ...
      <nav class="main-nav" aria-label="Main Navigation">
        ${navLinks.map((item) => `<a href="${item.href}" class="${item.href.endsWith(currentFile) ? 'active' : ''}">${item.label}</a>`).join('')}
  ...
  footer.innerHTML = `
    <div class="container footer-grid">
      ...
      <div>
        <h3>外部連結 External Link</h3>
        <a class="official-btn" href="https://hijo790401.github.io/shen-yao-portal/" target="_blank" rel="noopener noreferrer">官方網站 Official Portal</a>
      </div>
    </div>
    <div class="container footer-base">
      <small>© <span id="year"></span> 沈耀國際實相新聞台 × 責任博物館 | Last updated: <span id="last-updated"></span></small>
    </div>
  `;
  ...
  document.getElementById('last-updated').textContent = new Date(document.lastModified).toISOString().slice(0, 10);
})();
